...
export default function WindowChrome({
  ...
  const defaultX = initialX ?? (typeof window !== "undefined" ? Math.max(0, window.innerWidth / 2 - width / 2) : 100);
  const defaultY = initialY ?? (typeof window !== "undefined" ? Math.max(0, window.innerHeight / 2 - height / 2) : 100);

  const [bounds, setBounds] = useState({ x: defaultX, y: defaultY, w: width, h: height });
  ...
  return (
    ...
    >
      <Rnd
        ...
        position={maximized ? { x: 0, y: 0 } : { x: bounds.x, y: bounds.y }}
        onDragStop={(_, d) => {
          if (!maximized) setBounds((prev) => ({ ...prev, x: d.x, y: d.y }));
        }}
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, paddingLeft: 10, flex: 1, minWidth: 0 }}>
            ...
            <span style={{
              ...
              fontSize: 11, fontWeight: 400, letterSpacing: "0.01em", lineHeight: 1,
              overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
              fontFamily: "'Ubuntu', sans-serif", transition: "color 0.2s",
            }}>
              {title}
            </span>
          </div>
